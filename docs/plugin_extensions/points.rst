...
Reacting to Point Group score changes
*************************************

.. vale on

When a Contact's score within a Point Group changes, ``Mautic\PointBundle\Model\PointGroupModel::adjustPoints()`` saves the new score and dispatches the ``Mautic\PointBundle\PointGroupEvents::SCORE_CHANGE`` Event. Listeners receive a ``Mautic\PointBundle\Event\GroupScoreChangeEvent`` object that carries the Contact, the affected Point Group, and the old and new scores. Subscribe to this Event to run custom logic whenever a Contact's standing in a Point Group shifts.

.. php:namespace:: Mautic\PointBundle\Event
.. php:class:: GroupScoreChangeEvent

  GroupScoreChangeEvent class

  .. php:method:: public getContact()

      :return: The Contact whose Point Group score changed.
      :returntype: ``Mautic\LeadBundle\Entity\Lead``

  .. php:method:: public getGroup()

      :return: The Point Group whose score changed.
      :returntype: ``Mautic\PointBundle\Entity\Group``

  .. php:method:: public getOldScore()

      :return: The Contact's score in the Point Group before the change.
      :returntype: int

  .. php:method:: public getNewScore()

      :return: The Contact's score in the Point Group after the change.
      :returntype: int

  .. php:method:: public getGroupContactScore()

      :return: The underlying score record linking the Contact and Point Group.
      :returntype: ``Mautic\PointBundle\Entity\GroupContactScore``

Mautic's own Point Insights feature subscribes to this Event. ``Mautic\PointBundle\EventListener\PointInsightSubscriber`` listens for ``PointGroupEvents::SCORE_CHANGE`` and runs every published Point Insight for the Contact, so the winning Point Group stays up to date as scores change. Register a custom subscriber using the same pattern.
...
.. code-block:: php
    ...
    class PointGroupSubscriber implements EventSubscriberInterface
    {
        public static function getSubscribedEvents(): array
        {
            return [
        ...
        public function onGroupScoreChange(GroupScoreChangeEvent $event): void
        {
            ...
            $oldScore = $event->getOldScore();
            $newScore = $event->getNewScore();

            // Add custom logic to react to the score change.
        }
    }
